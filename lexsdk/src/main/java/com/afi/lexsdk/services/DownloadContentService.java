...
import com.afi.lexsdk.LexDownloadManager;
import com.afi.lexsdk.UtilityJava;
import com.afi.lexsdk.models.ContentDownloadEntity;
...
//import static com.infosysit.lexsdk.Constants.CONTENT_URL_PREFIX_REGEX;
...
//                    Log.d("DownloadContent",ex.getMessage());
            ...
            Log.d("DownloadContentOP","RECAHED");
            getCourseCall(mContentId).enqueue(new Callback<JsonObject>() {
                @Override
                public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                    if (response != null) {
                        try {
                            Log.d("LexTestingDownload","JSON: "+response.body());
                            hierarchyFetched(response.body().getAsJsonObject());
                        } catch (Exception ex) {
                                Log.d("DownloadContent",ex.getMessage());
                        }
                    }
                ...
                public void onFailure(Call<JsonObject> call, Throwable t) {
                    Log.d("DownloadContent", "Failure: "+t.getMessage());
                }
            });
        }
    }
...
//                ,Constants.UserEmail
    ...
    public Call<JsonObject> getCourseCall(String identifier) {
...
//        ApiInterfaceJava apiInterface = retrofit.create(ApiInterfaceJava.class);
        ...
        JsonParser parser = new JsonParser();
        JsonObject additionPAram = parser.parse("{'additionalFields':['averageRating','body','creatorContacts','creatorDetails','curatedTags','contentType','collections','hasTranslations','expiryDate','exclusiveContent','introductoryVideo','introductoryVideoIcon','isInIntranet','isTranslationOf','keywords','learningMode','playgroundResources','price','registrationInstructions','region','registrationUrl','resourceType','subtitle','softwareRequirements','systemRequirements','totalRating','uniqueLearners','viewCount']}").getAsJsonObject();
//        Log.d("DownloadContent","Hierarchy fetched " + apiInterface.);
        return mDownloadMode.equals("OPENRAP") ? apiInterface.getCourseHierarchyOpenRap() : apiInterface.getCourseHierarchyV2(additionPAram,identifier, UtilityJava.tokenValue, UtilityJava.widValue);
    }
